%% 
% This MATLAB script is used to implement Classifiers (SVM and kNN) on the selected bands result from GLBS. 
...
%          <ProjectRoot>/GLBS/results_India_150/GLBS_RES.mat
%        2. All the trained models of SVM and kNN for ten rouds
%          <ProjectRoot>/GLBS/models_India_150/Model.mat
%
...
%     fprintf(logfile, '\nIteration %d: \n',n);
    ...
    th = tic();
    [ bandset, S_ALL] = glbs(fit_class);
    t_glbssvm = toc(th);
    fprintf(logfile, 'GLBS_SVM Running Time: %.3fs\n',t_glbssvm);
    Model{n}.bands = S_ALL;
    fprintf(logfile, 'GLBS selected %d bands in sequence.\n',length(S_ALL));
    ...
    % save results, knn
    Model{n}.knn = knn_mdl;
    GLBS_RES{n}.knn.OA = OA_knn;
    GLBS_RES{n}.knn.Kappa = Kappa_knn;
    GLBS_RES{n}.knn.AA = AA_knn;
    % save results, svm
    Model{n}.svm = svm_mdl;
    GLBS_RES{n}.svm.OA = OA_svm;
    GLBS_RES{n}.svm.Kappa = Kappa_svm;
    GLBS_RES{n}.svm.AA = AA_svm;
end
